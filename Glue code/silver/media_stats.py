"""
AWS Glue Job: Wistia Silver — media_stats
==========================================
Reads today's bronze partition for media_stats, applies transformations,
and overwrites today's silver partition.

Triggered by: Glue Workflow, after the bronze job completes.

Transformations:
  - Rename: pipeline_media_id -> media_id
  - Cast:   pipeline_ingested_at_utc -> TIMESTAMP
"""
...
# Paths
# ---------------------------------------------------------------------------
BRONZE_PATH = f"s3://{S3_BUCKET}/{S3_PREFIX}/bronze/media_stats/ingested_date={TODAY}/"
SILVER_PATH = f"s3://{S3_BUCKET}/{S3_PREFIX}/silver/media_stats/ingested_date={TODAY}/"
...
# 1. Rename pipeline_media_id -> media_id
df = df.withColumnRenamed("pipeline_media_id", "media_id")

# 2. Cast pipeline_ingested_at_utc to timestamp
...
job.commit()
logger.info("Silver media_stats job completed successfully.")